pipeline {
    ...
    parameters {
        string(name: 'TAG_NAME', defaultValue: '', description: '镜像标签(vx.x.x用于发布正式版)')
    }

    environment {
        DOCKER_CREDENTIAL_ID = 'harbor-id'
        REGISTRY = 'hub.app-chengdu1.yunzhicloud.com'
        DOCKERHUB_NAMESPACE = 'yzcloud'
        APP_NAME = 'business-auth'
        GITLAB_CREDENTIAL_ID = 'gitlab-id'
        GIT_URL = 'gitlab.yunzhicloud.com/middle-platform/business-auth.git'     
        GIT_EMAIL = 'devops@yzcloud.com'
        // SONAR_CREDENTIAL_ID = 'sonar-token'
        KUBECONFIG_CREDENTIAL_ID = 'kube-id'
        PROD_KUBECONFIG_CREDENTIAL_ID = 'prod-kube-id'
    }
